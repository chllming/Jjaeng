# Agent Screen screen recording — implementation status
...
Agent Screen now has a working screen recording flow built into the app:
...
```text
crates/agent-screen-core/src/recording/mod.rs
```
...
Recording target selection reuses the existing screenshot selection plumbing in `crates/agent-screen-core/src/capture/mod.rs`.
...
The GTK app now owns recording lifecycle state in `crates/agent-screen-ui/src/app/mod.rs`.
...
```text
crates/agent-screen-ui/src/app/launchpad.rs
```
...
agent-screen --record-size=native|half|1080p|720p
agent-screen --record-encoding=standard|quality|small
agent-screen --record-audio=off|desktop|mic
agent-screen --record-mic=<source_name>
```

Implemented parsing lives in:

```text
crates/agent-screen-ui/src/app/runtime_support/startup.rs
crates/agent-screen-cli/src/main.rs
```
...
`crates/agent-screen-core/src/storage/mod.rs` now includes:
...
History is no longer screenshot-only. `crates/agent-screen-core/src/history.rs` now stores a mixed manifest using:
...
`crates/agent-screen-core/src/config/mod.rs` currently supports these recording fields:

```json
{
  "recording_dir": "~/Videos/agent-screen",
...
1. Split `crates/agent-screen-core/src/recording/mod.rs` into `mod.rs`, `session.rs`, and `audio.rs`.
