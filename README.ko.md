...
Jjaeng은 [ChalKak](https://github.com/BitYoungjae/ChalKak)을 기반으로 하며, 업스트림의 이중 라이선스를 그대로 유지합니다. 자세한 내용은 [NOTICE](NOTICE)를 참고하세요.
...
`Jjaeng`은 카메라 셔터 소리를 뜻하는 한국어 의성어 `찰칵!`에서 따왔습니다.
...
- `crates/jjaeng-core`: 공용 도메인 로직, 저장소, OCR, 테마, IPC, 제품 식별 정보.
- `crates/jjaeng-ui`: 런치패드, 미리보기, 편집기 창을 담당하는 GTK 런타임.
- `crates/jjaeng-daemon`: 백그라운드 실행과 런타임 제어를 위한 데몬 엔트리포인트.
- `crates/jjaeng-cli`: 사용자용 `jjaeng` 바이너리와 CLI 통합 테스트.
...
- Wayland + Hyprland 세션
- `HOME` 환경 변수 설정
- `XDG_RUNTIME_DIR` 권장 (없으면 `/tmp/jjaeng` 사용)
...
`jjaeng` 바이너리가 `~/.cargo/bin/`에 설치됩니다. 이 디렉터리가 `PATH`에 포함되어 있는지 확인하세요.
...
`jjaeng`를 플래그 없이 실행하면 시작 직후 종료됩니다. `--help`로 사용 가능한 옵션을 확인할 수 있습니다.
...
- `--launchpad`
- `--version` / `-V` — 버전 출력 (예: `Jjaeng 0.5.0 (abc1234)`)
- `--help` / `-h` — 사용법 출력
...
- `optdepends=('jjaeng-ocr-models: OCR 텍스트 인식 지원')`

패키지명 목표: `jjaeng`.

별도 AUR 패키지 `jjaeng-ocr-models`가 OCR용 PaddleOCR v5 모델 파일을 제공합니다. 패키징 메타데이터는 `aur/jjaeng-ocr-models/`에 있습니다.

## 출처

- 업스트림 프로젝트: `ChalKak`
- 업스트림 저장소: `https://github.com/BitYoungjae/ChalKak`
- 현재 저장소: `https://github.com/chllming/Jjaeng`

## 라이선스

`jjaeng`은 다음 이중 라이선스를 사용합니다.
