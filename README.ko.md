...
Agent Screen은 Wayland와 Hyprland 환경을 위한 스크린샷 및 녹화 도구입니다. 백그라운드 데몬, 좌하단 컴팩트 미리보기 흐름, 스크린샷/영상 히스토리, Omarchy 기반의 각진 평면 UI, 내장 주석 편집기를 제공합니다.

Agent Screen은 [ChalKak](https://github.com/BitYoungjae/ChalKak)에서 출발한 파생 프로젝트입니다. 업스트림 라이선스 모델은 그대로 유지하며, 출처 정보는 [NOTICE](NOTICE)에 정리되어 있습니다.

## 주요 기능

- 전체 화면, 영역, 창 캡처
- 전체 화면, 영역, 창 녹화와 크기/인코딩/오디오를 조정하는 녹화 프롬프트
- 데몬(`agent-screend`) 기반의 백그라운드 실행과 소켓 제어
...
- `crates/agent-screen-core`: 캡처, 저장소, 클립보드, OCR, IPC, 히스토리, 공용 서비스
- `crates/agent-screen-ui`: 미리보기, 히스토리, 런치패드, 편집기용 GTK 런타임
- `crates/agent-screen-daemon`: 숨김 데몬 바이너리 `agent-screend`
- `crates/agent-screen-cli`: 사용자용 CLI 바이너리 `agent-screen`
...
- `slurp`
- `wl-clipboard`
- `gpu-screen-recorder` 또는 `wl-screenrec` (영상 녹화용)
- `pactl` (녹화 오디오 소스 탐지용)
- GTK4 런타임 라이브러리
...
- Waybar 상태 스크립트: [scripts/agent-screen-waybar-status.sh](scripts/agent-screen-waybar-status.sh)
- Omarchy/Hyprland 연동은 `~/.config` 오버라이드에서 설정하는 것을 전제로 합니다
- Omarchy가 설치되어 있으면 Agent Screen은 현재 Omarchy 팔레트와 메뉴 타이포그래피를 런타임 기본 테마로 읽습니다
...
- `screenshot_dir`: 기본 스크린샷 저장 폴더를 덮어씀 (기본값: `$HOME/Pictures`)
- `recording_dir`: 기본 녹화 저장 폴더를 덮어씀 (기본값: `$HOME/Videos`)
