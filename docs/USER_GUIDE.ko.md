...
5. 미리보기에서: `s` 저장, `c` 복사, `e` 또는 `더블클릭`으로 편집기 열기.
...
- 미리보기를 한 번 클릭하면 해당 창에 포커스가 가서 단축키가 바로 동작합니다.
- 미리보기 이미지를 더블클릭하면 해당 캡처가 편집기로 열립니다.
- 미리보기 창은 작업 공간을 가리지 않도록 활성 모니터의 좌하단 근처에 작게 배치됩니다.
...
|-----------|-----------|------|--------|
| PNG 이미지 | `image/png` | PNG 이미지 바이트 | 이미지 편집기, 브라우저, 채팅 앱, 코딩 에이전트 |
| 기타 파일 | `text/plain;charset=utf-8` | 절대 파일 경로 (UTF-8) | 텍스트 편집기, 터미널, 파일 관리자 |

스크린샷 캡처(항상 PNG)의 경우, 이미지를 인식하는 앱에 원본 이미지 데이터가 직접 전달됩니다. 중간 파일 경로 없이 브라우저, 채팅 앱, 코딩 에이전트에 바로 붙여넣기가 가능합니다.
