# Agent Screen Refactor Plan

## Goal

Refactor Agent Screen from a preview-first GTK app into a background screenshot service with an optional UI layer that fits Omarchy's Hyprland + Waybar desktop model.
...
Create `agent-screen-core` and move these concerns out of the GTK app:
...
Build `agent-screend` around a local Unix socket.
...
- click handlers call `agent-screenctl`
...
Treat Agent Screen as a background screenshot service with a compact action preview and a separate editor, not as a normal windowed app that is later hidden into the status bar.
