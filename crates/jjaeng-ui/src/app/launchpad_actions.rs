...
pub(super) struct LaunchpadActionExecutor {
    ...
    ocr_engine: Rc<RefCell<Option<jjaeng_core::ocr::OcrEngine>>>,
    ocr_language: jjaeng_core::ocr::OcrLanguage,
    ...
    pub(super) fn capture_and_open_preview(
        ...
        match capture_result {
            Ok(artifact) => {
                ...
                jjaeng_core::notification::send(success_toast_message);
            }
            Err(err) => {
                set_status(&self.status_log, format!("{failure_status_prefix}: {err}"));
                jjaeng_core::notification::send(format!("{failure_toast_prefix}: {err}"));
            }
        }
    }
    ...
    pub(super) fn run_preview_ocr_action(&self) {
        ...
        spawn_worker_action(
            move || {
                ...
                let result = jjaeng_core::ocr::recognize_text_from_file(&engine, &temp_path);
                (Some(engine), result)
            },
            move |(engine, result): (
                Option<jjaeng_core::ocr::OcrEngine>,
                Result<String, jjaeng_core::ocr::OcrError>,
            )| {
                ...
                match result {
                    ...
                    Err(err) => {
                        ...
                        jjaeng_core::notification::send(format!("OCR failed: {err}"));
                    }
                }
            },
        );
...
fn apply_preview_action_result(
    ...
    if matches!(action, PreviewAction::Save | PreviewAction::Copy) {
        jjaeng_core::notification::send(outcome.toast_message);
    } else {
...
mod tests {
    ...
    fn preview_action_ui_outcome_reports_error_for_failed_copy() {
        let result = Err(PreviewActionError::ClipboardError {
            ...
            source: jjaeng_core::clipboard::ClipboardError::CommandIo {
