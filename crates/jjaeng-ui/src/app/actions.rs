...
use jjaeng_core::capture::CaptureArtifact;
use jjaeng_core::clipboard::ClipboardBackend;
use jjaeng_core::editor::{EditorAction, EditorActionError, EditorEvent};
use jjaeng_core::preview::{PreviewAction, PreviewActionError, PreviewEvent};
use jjaeng_core::storage::CaptureStorage;
...
mod tests {
    ...
    impl ClipboardBackend for FakeClipboard {
        fn copy(&self, path: &Path) -> jjaeng_core::clipboard::ClipboardResult<()> {
    ...
    impl CaptureStorage for FakeStorage {
        fn save_capture(
            &self,
            artifact: &CaptureArtifact,
        ) -> jjaeng_core::storage::StorageResult<PathBuf> {
            self.save_requests
                .borrow_mut()
        ...
        fn discard_session_artifacts(
            &self,
            capture_id: &str,
        ) -> jjaeng_core::storage::StorageResult<()> {
