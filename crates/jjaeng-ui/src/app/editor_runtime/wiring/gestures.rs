...
use jjaeng_core::editor::tools::{CropElement, RectangleElement};
use jjaeng_core::editor::{self, ToolKind, ToolObject};
...
pub(in crate::app::editor_runtime) struct EditorDrawGestureContext {
    ...
    pub(in crate::app::editor_runtime) ocr_engine: Rc<RefCell<Option<jjaeng_core::ocr::OcrEngine>>>,
    pub(in crate::app::editor_runtime) ocr_language: jjaeng_core::ocr::OcrLanguage,
    ...
    *context.status_log_for_render.borrow_mut() =
    ...
    spawn_worker_action(
        move || {
            ...
            let result = jjaeng_core::ocr::recognize_text(&engine, &image);
            (Some(engine), result)
        },
        move |(engine, result): (
