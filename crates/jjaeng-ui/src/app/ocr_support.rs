...
pub(super) fn resolve_or_init_engine(
    engine: Option<jjaeng_core::ocr::OcrEngine>,
    language: jjaeng_core::ocr::OcrLanguage,
) -> Result<jjaeng_core::ocr::OcrEngine, jjaeng_core::ocr::OcrError> {
    if let Some(engine) = engine {
        return Ok(engine);
    }

    let model_dir = jjaeng_core::ocr::resolve_model_dir().ok_or_else(|| {
        jjaeng_core::ocr::OcrError::EngineInit {
            message: "model directory not found".to_string(),
        }
    })?;
    jjaeng_core::ocr::create_engine(&model_dir, language)
}
...
pub(super) fn pixbuf_region_to_dynamic_image(
    ...
    width: u32,
    height: u32,
) -> jjaeng_core::ocr::OcrResult<image::DynamicImage> {
    if width == 0 || height == 0 {
        return Err(jjaeng_core::ocr::OcrError::InvalidRegion {
    ...
    let img_buf = image::RgbImage::from_raw(clamped_w, clamped_h, rgb_buf).ok_or_else(|| {
        jjaeng_core::ocr::OcrError::ImageConversion {
            message: format!(
...
pub(super) fn handle_ocr_text_result(status_log: &SharedStatusLog, text: String) {
    if text.is_empty() {
        ...
        jjaeng_core::notification::send("No text found");
        return;
    }
    ...
    jjaeng_core::notification::send(format!("Copied: {preview_text}"));
}
...
mod tests {
    ...
    fn resolve_or_init_engine_errors_when_no_model_dir() {
        ...
        let result = resolve_or_init_engine(None, jjaeng_core::ocr::OcrLanguage::English);
        ...
        if !std::path::Path::new(jjaeng_core::identity::DEFAULT_SYSTEM_MODEL_DIR).is_dir()
            && !std::path::Path::new(jjaeng_core::identity::LEGACY_SYSTEM_MODEL_DIR).is_dir()
        {
    ...
    fn pixbuf_region_rejects_zero_size() {
        ...
        assert!(matches!(
            result,
            Err(jjaeng_core::ocr::OcrError::InvalidRegion { .. })
        ));
    }
}
