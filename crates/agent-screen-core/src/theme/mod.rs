...
/// Typed subset of Omarchy Quattro's generated `shell.toml` consumed by GTK.
/// Unknown sections and keys remain forward-compatible and are ignored.
#[derive(Debug, Clone, PartialEq)]
pub struct OmarchyShellStyle {
    pub font_family: String,
    pub base_font_size_px: u16,
    pub spacing_scale: f32,
    pub control_height_px: u16,
    pub panel_padding_px: u16,
...
impl Default for OmarchyShellStyle {
    fn default() -> Self {
        Self {
            // shell.toml does not require a family; the generated walker style
            // is the Quattro source of truth when this remains empty.
...
pub fn load_omarchy_color_tokens() -> Option<ColorTokens> {
    ...
    load_omarchy_color_tokens_with(state_home.as_deref(), None)
}

pub fn load_omarchy_shell_style() -> OmarchyShellStyle {
    let home = std::env::var_os("HOME").map(PathBuf::from);
    let state_home = std::env::var_os("XDG_STATE_HOME")
        .map(PathBuf::from)
        .filter(|path| !path.as_os_str().is_empty())
        .or_else(|| home.as_ref().map(|path| path.join(".local/state")));
    let generated = state_home
        .as_deref()
        .map(|root| root.join("omarchy/current/theme/shell.toml"));
    let user_override = std::env::var_os("XDG_CONFIG_HOME")
        .map(PathBuf::from)
        .filter(|path| !path.as_os_str().is_empty())
        .or_else(|| home.as_ref().map(|path| path.join(".config")))
        .map(|root| root.join("omarchy/shell.toml"));
    load_omarchy_shell_style_from_paths(generated.as_deref(), user_override.as_deref())
}

fn load_omarchy_shell_style_from_paths(
    generated: Option<&Path>,
    user_override: Option<&Path>,
) -> OmarchyShellStyle {
    let mut style = OmarchyShellStyle::default();
    for path in [generated, user_override].into_iter().flatten() {
        if let Ok(serialized) = fs::read_to_string(path) {
            apply_shell_style_toml(&mut style, &serialized);
        }
    }
    style
}

fn apply_shell_style_toml(style: &mut OmarchyShellStyle, serialized: &str) {
    let Ok(value) = serialized.parse::<toml::Value>() else {
        tracing::debug!("failed to parse Omarchy shell.toml");
        return;
    };
    ...
    if let Some(value) = get("spacing", "scale").and_then(toml::Value::as_float) {
        style.spacing_scale = (value as f32).clamp(0.5, 2.0);
    }
    if let Some(value) = get("spacing", "control-height").and_then(toml::Value::as_integer) {
        style.control_height_px = (value as u16).clamp(20, 96);
    }
    if let Some(value) = get("spacing", "panel-padding").and_then(toml::Value::as_integer) {
        style.panel_padding_px = (value as u16).clamp(0, 64);
    }
    if let Some(value) = get("font", "base-size").and_then(toml::Value::as_integer) {
        style.base_font_size_px = (value as u16).clamp(8, 32);
    }
    if let Some(value) = get("font", "family").and_then(toml::Value::as_str) {
        style.font_family = value.to_string();
    }
    if let Some(value) = get("controls", "normal-border-width").and_then(toml::Value::as_integer) {
        style.border_width_px = (value as u16).clamp(1, 4);
    }
    for (key, target) in [
        ("normal-fill-alpha", &mut style.normal_fill_alpha),
        ("hover-cursor-fill-alpha", &mut style.hover_fill_alpha),
        ("focus-fill-alpha", &mut style.focus_fill_alpha),
        ("selected-fill-alpha", &mut style.selected_fill_alpha),
    ] {
        if let Some(value) = get("controls", key).and_then(toml::Value::as_float) {
            *target = (value as f32).clamp(0.0, 1.0);
        }
    }
    if let Some(value) = get("bar", "active").and_then(toml::Value::as_str) {
        style.active_color = value.to_string();
    }
}
...
fn theme_config_path_with(
    ...
    existing_app_config_path(THEME_CONFIG_FILE, xdg_config_home, home).map_err(
        |error| match error {
            ConfigPathError::MissingHomeDirectory => ThemeError::MissingHomeDirectory,
        },
    )
...
mod tests {
    ...
    fn fixture_root() -> PathBuf {
        ...
        path.push(format!("{APP_SLUG}-theme-{pid}-{nanos}"));
        path
    }
