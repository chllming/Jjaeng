...
use crate::identity::{
    APP_SLUG, DEFAULT_SYSTEM_MODEL_DIR, LEGACY_APP_SLUG, LEGACY_SYSTEM_MODEL_DIR, UPSTREAM_SLUG,
    UPSTREAM_SYSTEM_MODEL_DIR,
};
...
pub fn resolve_model_dir() -> Option<PathBuf> {
    ...
    if let Some(base) = data_root {
        for candidate in [APP_SLUG, LEGACY_APP_SLUG, UPSTREAM_SLUG] {
            let dir = base.join(candidate).join("models");
            if dir.is_dir() {
                return Some(dir);
            }
        }
    ...
    for system_dir in [
        DEFAULT_SYSTEM_MODEL_DIR,
        LEGACY_SYSTEM_MODEL_DIR,
        UPSTREAM_SYSTEM_MODEL_DIR,
    ] {
...
mod tests {
    ...
    fn resolve_model_dir_returns_none_when_no_dirs_exist() {
        ...
        if !PathBuf::from(DEFAULT_SYSTEM_MODEL_DIR).is_dir()
            && !PathBuf::from(LEGACY_SYSTEM_MODEL_DIR).is_dir()
        {
    ...
    fn resolve_model_dir_prefers_user_dir_over_system() {
        let tmp = std::env::temp_dir().join("jjaeng-test-user-models");
        let user_model_dir = tmp.join(APP_SLUG).join("models");
        let _ = std::fs::create_dir_all(&user_model_dir);
