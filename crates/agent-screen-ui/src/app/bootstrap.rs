...
use agent_screen_core::config::load_app_config;
use agent_screen_core::editor::tools::Color;
use agent_screen_core::input::{load_editor_navigation_bindings, EditorNavigationBindings};
use agent_screen_core::storage::prune_stale_temp_files;
use agent_screen_core::theme::{
    load_omarchy_color_tokens, load_omarchy_shell_style, load_theme_config,
    resolve_color_tokens_with_base, resolve_editor_defaults, EditorDefaults, OmarchyShellStyle,
...
pub(super) struct ResolvedThemeRuntime {
    ...
    pub(super) shell_style: OmarchyShellStyle,
    ...
    pub(super) ocr_language: agent_screen_core::ocr::OcrLanguage,
}
...
pub(super) fn bootstrap_app_runtime(startup_override: Option<StartupConfig>) -> AppBootstrap {
    if let Err(err) = agent_screen_core::migration::migrate_legacy_state() {
        tracing::warn!(
            ?err,
            "failed to migrate legacy Agent Screen state; continuing"
        );
    }
    let startup_config = startup_override.unwrap_or_else(StartupConfig::from_args);
...
pub(super) fn resolve_theme_runtime(
    ...
    let base_color_tokens = load_omarchy_color_tokens()
        .unwrap_or_else(|| agent_screen_core::theme::default_color_tokens(mode));
    let shell_style = load_omarchy_shell_style();
    ...
    let ocr_language =
        agent_screen_core::ocr::resolve_ocr_language(app_config.ocr_language.as_deref());
...
mod tests {
    ...
    fn resolve_theme_runtime_uses_mode_specific_editor_defaults() {
        let config = ThemeConfig {
            ...
            editor_modes: Some(agent_screen_core::theme::EditorModeDefaults {
                dark: EditorDefaults {
