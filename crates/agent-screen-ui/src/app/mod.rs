...
use agent_screen_core::capture;
use agent_screen_core::clipboard::{ClipboardBackend, WlCopyBackend};
use agent_screen_core::editor::tools::CropElement;
use agent_screen_core::editor::{self, EditorAction, ToolKind};
use agent_screen_core::error::AppResult;
use agent_screen_core::history::HistoryService;
use agent_screen_core::identity::{APP_CSS_ROOT, APP_ID, APP_NAME};
use agent_screen_core::input::ShortcutAction;
use agent_screen_core::preview::PreviewAction;
use agent_screen_core::recording::{
    self, AudioMode, RecordingEncodingPreset, RecordingRequest, RecordingSelection, RecordingSize,
};
use agent_screen_core::service::{RemoteCommand, StatusSnapshot};
use agent_screen_core::state::{AppEvent, AppState, StateMachine};
use agent_screen_core::storage::StorageService;
use gtk4::prelude::*;
...
impl App {
    ...
    pub fn start(&mut self, startup_override: Option<StartupConfig>) -> AppResult<()> {
        ...
        let app_config = agent_screen_core::config::load_app_config();

        if print_status_json {
            match agent_screen_core::service::read_status_snapshot_json() {
        ...
        let local_startup_remote_command = if daemon_mode {
            ...
            match agent_screen_core::service::try_send_command(&command) {
        ...
        application.connect_activate(move |app| {
            ...
            let shell_style = resolved_theme_runtime.shell_style;
            ...
            install_runtime_css(style_tokens, &color_tokens, &shell_style, motion_enabled);
            watch_runtime_css(
                style_tokens,
                color_tokens.clone(),
                theme_mode,
                theme_config.colors.clone(),
                motion_enabled,
            );
            ...
            let settings_info = {
                ...
                let ocr_model_dir_label = agent_screen_core::ocr::resolve_model_dir()
                    .map(|p| p.display().to_string())
                    .unwrap_or_else(|| "not found".to_string());
                let (xdg_config, home_dir) = agent_screen_core::config::config_env_dirs();
                let theme_config_path = agent_screen_core::config::existing_app_config_path(
                    "theme.json",
                    xdg_config.as_deref(),
                    home_dir.as_deref(),
                )
                ...
                let config_path = agent_screen_core::config::existing_app_config_path(
                    "config.json",
                    xdg_config.as_deref(),
                    home_dir.as_deref(),
                )
                .map(|p| p.display().to_string())
                .unwrap_or_else(|_| "unavailable".to_string());
                let keybinding_config_path = agent_screen_core::config::existing_app_config_path(
                    "keybindings.json",
                    xdg_config.as_deref(),
                    home_dir.as_deref(),
                )
            ...
            let ocr_available = agent_screen_core::ocr::resolve_model_dir().is_some();
            let ocr_engine: Rc<RefCell<Option<agent_screen_core::ocr::OcrEngine>>> =
            ...
            let render: Rc<dyn Fn()> = {
                ...
                Rc::new(move || {
                    ...
                    if let Ok(snapshot_json) = serde_json::to_string(&status_snapshot) {
                        ...
                        if changed || elapsed {
                            if let Err(err) =
                                agent_screen_core::service::write_status_snapshot(&status_snapshot)
                            {
                    ...
                    move |request: RecordingRequest,
                        ...
                        if !recording::recording_backend_available() {
                            ...
                            *status_log.borrow_mut() = message.clone();
                            ...
                            agent_screen_core::notification::send(message);
                            if let Some(render) = render_handle.borrow().as_ref() {
                                (render.as_ref())();
                            }
                            return;
                                    ...
                                    Ok(handle) => {
                                            ...
                                            machine.borrow_mut().transition(AppEvent::StartRecording)
                                        {
                                            ...
                                            *status_log.borrow_mut() =
                                            ...
                                            agent_screen_core::notification::send("Recording started");
                                        }
                                    }
                                    Err(err) => {
                                        recording_flow_pending.set(false);
                                        *status_log.borrow_mut() =
                                        ...
                                        agent_screen_core::notification::send(format!(
                    ...
                    let result = if active.is_paused() {
                        recording::resume_recording(&active.handle).map(|_| {
                            ...
                            *status_log.borrow_mut() = "recording resumed".to_string();
                            agent_screen_core::notification::send("Recording resumed");
                        })
                    } else {
                        recording::pause_recording(&active.handle).map(|_| {
                            active.mark_paused();
                            *status_log.borrow_mut() = "recording paused".to_string();
                            agent_screen_core::notification::send("Recording paused");
                        })
                    };
                    ...
                    match recording::stop_recording(&mut active.handle) {
                        Ok(artifact) => {
                            ...
                            if result_output_path.exists() && result_thumbnail_path.exists() {
                                    ...
                                    Rc::new(move || {
                                        let Some(storage_service) =
                                            ...
                                            *status_log.borrow_mut() =
                                            ...
                                            agent_screen_core::notification::send(
                                ...
                                if persisted {
                                    agent_screen_core::notification::send("Recording ready");
                                } else {
                                    agent_screen_core::notification::send(format!(
                                ...
                                agent_screen_core::notification::send("Recording stopped");
                            } else {
                                agent_screen_core::notification::send(format!(
                                    "Recording stopped; file kept at {}",
                                    artifact.output_path.display()
                        ...
                        Err(err) => {
                            if should_restore_active_recording_on_stop_error(&err) {
                                *status_log.borrow_mut() =
                                ...
                                agent_screen_core::notification::send(if output_exists {
                                    format!(
                                        "Recording stopped; file kept at {}",
                                        output_path.display()
                                    )
            ...
            if daemon_mode {
                if let Some(command_server_guard) =
                    agent_screen_core::service::spawn_command_server(remote_command_tx.clone())
                {
