...
use agent_screen_core::input::{resolve_shortcut, InputContext, InputMode, ShortcutKey};
...
pub(super) fn build_editor_window_shell(
    ...
    editor_window_instance.add_css_class(agent_screen_core::identity::APP_CSS_ROOT);
...
fn resolve_editor_tool_fallback_shortcut(shortcut_key: ShortcutKey) -> Option<ToolKind> {
    ...
    let action = resolve_shortcut(
        shortcut_key,
        agent_screen_core::input::ShortcutModifiers::default(),
        InputContext {
