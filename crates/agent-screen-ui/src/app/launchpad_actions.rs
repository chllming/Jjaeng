...
use agent_screen_core::clipboard::WlCopyBackend;
use agent_screen_core::history::HistoryService;
use agent_screen_core::preview::{PreviewAction, PreviewActionError, PreviewEvent};
...
pub(super) struct LaunchpadActionExecutor {
    ...
    ocr_engine: Rc<RefCell<Option<agent_screen_core::ocr::OcrEngine>>>,
    ocr_language: agent_screen_core::ocr::OcrLanguage,
    ...
    pub(super) fn capture_and_open_preview(
        ...
        match capture_result {
            Ok(artifact) => {
                ...
                agent_screen_core::notification::send(success_toast_message);
            }
            Err(err) => {
                set_status(&self.status_log, format!("{failure_status_prefix}: {err}"));
                agent_screen_core::notification::send(format!("{failure_toast_prefix}: {err}"));
            }
        }
    }
    ...
    pub(super) fn run_preview_ocr_action(&self) {
        ...
        spawn_worker_action(
            move || {
                ...
                let result = agent_screen_core::ocr::recognize_text_from_file(&engine, &temp_path);
                (Some(engine), result)
            },
            move |(engine, result): (
                Option<agent_screen_core::ocr::OcrEngine>,
                Result<String, agent_screen_core::ocr::OcrError>,
            )| {
                ...
                match result {
                    ...
                    Err(err) => {
                        ...
                        agent_screen_core::notification::send(format!("OCR failed: {err}"));
                    }
                }
            },
        );
        ...
        if let Some(service) = self.storage_service.as_ref() {
            if let Err(err) = service.discard_session_artifacts(capture_id) {
                tracing::warn!(
                    capture_id = %capture_id,
                    ?err,
                    "failed to discard temporary artifact after completed preview action"
                );
            }
        }
        close_preview_window_for_capture(
...
fn apply_preview_action_result(
    ...
    if matches!(action, PreviewAction::Save | PreviewAction::Copy) {
        agent_screen_core::notification::send(outcome.toast_message);
    } else {
...
mod tests {
    ...
    fn preview_action_ui_outcome_reports_error_for_failed_copy() {
        let result = Err(PreviewActionError::ClipboardError {
            ...
            source: agent_screen_core::clipboard::ClipboardError::CommandIo {
