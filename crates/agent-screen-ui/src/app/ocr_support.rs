...
pub(super) fn resolve_or_init_engine(
    engine: Option<agent_screen_core::ocr::OcrEngine>,
    language: agent_screen_core::ocr::OcrLanguage,
) -> Result<agent_screen_core::ocr::OcrEngine, agent_screen_core::ocr::OcrError> {
    if let Some(engine) = engine {
        return Ok(engine);
    }

    let model_dir = agent_screen_core::ocr::resolve_model_dir().ok_or_else(|| {
        agent_screen_core::ocr::OcrError::EngineInit {
            message: "model directory not found".to_string(),
        }
    })?;
    agent_screen_core::ocr::create_engine(&model_dir, language)
}
...
pub(super) fn pixbuf_region_to_dynamic_image(
    ...
    width: u32,
    height: u32,
) -> agent_screen_core::ocr::OcrResult<image::DynamicImage> {
    if width == 0 || height == 0 {
        return Err(agent_screen_core::ocr::OcrError::InvalidRegion {
    ...
    let img_buf = image::RgbImage::from_raw(clamped_w, clamped_h, rgb_buf).ok_or_else(|| {
        agent_screen_core::ocr::OcrError::ImageConversion {
            message: format!(
...
pub(super) fn handle_ocr_text_result(status_log: &SharedStatusLog, text: String) {
    if text.is_empty() {
        ...
        agent_screen_core::notification::send("No text found");
        return;
    }
    ...
    agent_screen_core::notification::send(format!("Copied: {preview_text}"));
}
...
mod tests {
    ...
    fn resolve_or_init_engine_errors_when_no_model_dir() {
        ...
        // (`/usr/share/jjaeng/models` or the legacy `/usr/share/chalkak/models`)
        // may still exist on dev machines, so
        // skip the assertion when it does.
        let prev_xdg = std::env::var("XDG_DATA_HOME").ok();
        let prev_home = std::env::var("HOME").ok();
        std::env::set_var("XDG_DATA_HOME", "/tmp/jjaeng-test-nonexistent-dir");
        std::env::set_var("HOME", "/tmp/jjaeng-test-nonexistent-home");

        let result = resolve_or_init_engine(None, agent_screen_core::ocr::OcrLanguage::English);
        ...
        if !std::path::Path::new(agent_screen_core::identity::DEFAULT_SYSTEM_MODEL_DIR).is_dir()
            && !std::path::Path::new(agent_screen_core::identity::LEGACY_SYSTEM_MODEL_DIR).is_dir()
        {
    ...
    fn pixbuf_region_rejects_zero_size() {
        ...
        assert!(matches!(
            result,
            Err(agent_screen_core::ocr::OcrError::InvalidRegion { .. })
        ));
    }
}
