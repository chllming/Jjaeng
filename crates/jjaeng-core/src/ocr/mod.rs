...
use crate::identity::{APP_SLUG, DEFAULT_SYSTEM_MODEL_DIR, LEGACY_SYSTEM_MODEL_DIR, UPSTREAM_SLUG};
...
pub fn resolve_model_dir() -> Option<PathBuf> {
    ...
    if let Some(base) = data_root {
        for candidate in [APP_SLUG, UPSTREAM_SLUG] {
    ...
    for system_dir in [DEFAULT_SYSTEM_MODEL_DIR, LEGACY_SYSTEM_MODEL_DIR] {
