...
fn resolve_pictures_dir(home: &Path, configured_pictures_dir: Option<PathBuf>) -> PathBuf {
    match configured_pictures_dir {
        Some(path) if path.is_absolute() => path,
        Some(path) => home.join(path),
        None => home.join(PREVIEW_SUBDIR),
    }
}
...
mod tests {
    ...
    #[test]
    fn resolve_pictures_dir_prefers_configured_relative_path() {
        let path = resolve_pictures_dir(Path::new("/home/test"), Some(PathBuf::from("Shots")));
        assert_eq!(path, PathBuf::from("/home/test/Shots"));
    }

    #[test]
    fn resolve_pictures_dir_prefers_configured_absolute_path() {
        let path = resolve_pictures_dir(
            Path::new("/home/test"),
