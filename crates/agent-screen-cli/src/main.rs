fn main() {
    if let Err(err) = agent_screen_core::migration::migrate_legacy_state() {
        eprintln!("Agent Screen migration warning: {err}");
    }
    if let Some(code) = handle_early_args() {
        std::process::exit(code);
    }

    if let Err(err) = agent_screen_ui::run() {
        eprintln!("Agent Screen failed: {err}");
        std::process::exit(1);
    }
}
...
fn handle_early_args() -> Option<i32> {
    ...
    match arg.as_str() {
        "-V" => {
            println!("Agent Screen {}", env!("CARGO_PKG_VERSION"));
            Some(0)
        }
        "--version" => {
            ...
            if git_hash.is_empty() {
                ...
                println!("Agent Screen {} ({git_hash})", env!("CARGO_PKG_VERSION"));
            }
            Some(0)
        }
        "-h" | "--help" => {
        ...
        "--status-json" => match agent_screen_core::service::read_status_snapshot_json() {
            Ok(json) => {
                println!("{json}");
                Some(0)
            }
            Err(err) => {
                eprintln!("Agent Screen status read failed: {err}");
                Some(1)
            }
        },
        _ => None,
...
fn print_help() {
    println!(
        "\
Agent Screen — Hyprland screenshot preview and editor utility

Usage: agent-screen [OPTIONS]
