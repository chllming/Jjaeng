...
impl JjaengServer {
    #[tool(
        name = "status",
        description = "Return Jjaeng daemon and MCP recording status"
    )]
    pub async fn status(&self, _params: Parameters<EmptyParams>) -> Result<Json<Value>, String> {
    ...
    pub async fn list_monitors(
        ...
        Ok(Json(json!(hyprland_monitors()?)))
    }
    ...
    pub async fn list_workspaces(
        ...
        Ok(Json(json!(hyprland_workspaces()?)))
    }
    ...
    #[tool(
        name = "history",
        description = "List Jjaeng screenshot and recording history"
    )]
    pub async fn history(&self, params: Parameters<HistoryParams>) -> Result<Json<Value>, String> {
    ...
    pub async fn recording_status(
        ...
        recording_status_value(&self.runtime).map(Json)
    }
    ...
    pub async fn pause_recording(
        ...
        Ok(Json(json!({"ok": true, "state": "paused"})))
    }
    ...
    pub async fn resume_recording(
        ...
        Ok(Json(json!({"ok": true, "state": "recording"})))
    }

    #[tool(
        name = "open_history",
        description = "Open the existing Jjaeng history window"
    )]
    pub async fn open_history(
        &self,
        _params: Parameters<EmptyParams>,
    ) -> Result<Json<Value>, String> {
        daemon_action("OpenHistory").map(Json)
    }

    #[tool(name = "open_preview", description = "Open the latest Jjaeng preview")]
    pub async fn open_preview(
        &self,
        _params: Parameters<EmptyParams>,
    ) -> Result<Json<Value>, String> {
        daemon_action("OpenPreview").map(Json)
    }

    #[tool(
        name = "open_editor",
        description = "Open the latest Jjaeng capture in the editor"
    )]
    pub async fn open_editor(
        &self,
        _params: Parameters<EmptyParams>,
    ) -> Result<Json<Value>, String> {
        daemon_action("OpenEditor").map(Json)
    }

    #[tool(
        name = "copy_latest",
        description = "Copy the latest Jjaeng capture to the clipboard"
    )]
    pub async fn copy_latest(
        &self,
        _params: Parameters<EmptyParams>,
    ) -> Result<Json<Value>, String> {
        daemon_action("CopyLatest").map(Json)
    }

    #[tool(name = "save_latest", description = "Save the latest Jjaeng capture")]
    pub async fn save_latest(
        &self,
        _params: Parameters<EmptyParams>,
    ) -> Result<Json<Value>, String> {
        daemon_action("SaveLatest").map(Json)
    }

    #[tool(
        name = "dismiss_latest",
        description = "Dismiss the latest Jjaeng preview"
    )]
    pub async fn dismiss_latest(
        &self,
        _params: Parameters<EmptyParams>,
    ) -> Result<Json<Value>, String> {
        daemon_action("DismissLatest").map(Json)
    }
}
...
impl ServerHandler for JjaengServer {
    ...
    async fn get_prompt(
        ...
        let text = match request.name.as_str() {
            ...
            "research" => "Research online with the host web tools while keeping local Jjaeng screenshots and recordings private. Preserve URLs and dates.",
            _ => return Err(ErrorData::invalid_params("unknown Jjaeng prompt", None)),
        };
...
fn daemon_action(variant: &str) -> Result<Value, String> {
    ...
    let mut stream = UnixStream::connect(daemon_socket_path())
        .map_err(|err| format!("Jjaeng daemon socket unavailable: {err}"))?;
    stream
...
fn read_daemon_status() -> Result<Value, String> {
    let path = runtime_dir().join("jjaeng-status.json");
...
fn runtime_dir() -> PathBuf {
    std::env::var_os("XDG_RUNTIME_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("/tmp/jjaeng"))
}
...
fn history_root() -> PathBuf {
    state_root().join("jjaeng/history")
}
...
fn find_history_entry(id: &str) -> Result<HistoryEntry, String> {
    ...
    let state_media_root = state_root().join("jjaeng");
    let cache_media_root = cache_root().join("jjaeng");
    if !entry.media_path.starts_with(&state_media_root)
        || (!entry.thumbnail_path.starts_with(&cache_media_root)
            && !entry.thumbnail_path.starts_with(&state_media_root))
    {
        return Err("artifact path is outside Jjaeng-managed storage".to_string());
    }
    Ok(entry)
}
